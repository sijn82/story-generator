...
export default function Card({card, showCard, setHand, zoom}) {
    return (
        <>
            ...
            <div className={`rounded p-0.5 tall:p-2  border border-2 border-gray-200 text-gray-600 ${card.in_play ? 'bg-gray-100' : 'bg-gray-600'} tall:h-32 medium:h-20 h-16 2xl:h-24 aspect-[3/4] grid grid-rows-4 ${zoom ? 'lg:h-24' : ''}`} 
                ...
                <div className={`relative flex row-span-2`}>
                    <div 
                        ...
                        className={` bg-no-repeat absolute bg-contain top-0 right-0 tall:h-8 tall:w-8 medium:h-8 medium:w-8 h-6 w-6 xl:h-8 xl:h-8 `}>
